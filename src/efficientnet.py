...
from src.layers.squeeze_excite import SqueezeExcite
from src.layers.activation import Swish
from src.layers.drop_path import DropPath
from src.layerutils import load_pretrained, make_divisible
...
class EfficientNet(nn.Cell):
    ...
    def __init__(self,
        ...
        layers_2 = layers[4:5]
        layers_3 = layers[5:6]
        layers_ap = layers[6:8]
        layers_4 = layers[8:9]
        layers_5 = layers[9:10]
        self.features = nn.SequentialCell(layers_1)
        self.features_2 = nn.SequentialCell(layers_2)
        self.features_3 = nn.SequentialCell(layers_3)
        self.features_ap = nn.SequentialCell(layers_ap)
        self.features_4 = nn.SequentialCell(layers_4)
        self.features_5 = nn.SequentialCell(layers_5)
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        xap = self.features_ap(x2)
        x3 = self.features_4(xap)
        x4 = self.features_5(x3)
        # return self.forward_head(x)
        return [x, x1, x2, xap, x4]
